...
class ForgotPasswordRequestModel extends ActiveRecord
{
  ...
  const ALERTTYPE_REQUEST_BY_EMAIL  = 'forgotPassByEmail';
  const ALERTTYPE_REQUEST_BY_MOBILE = 'forgotPassByMobile';
  const ALERTTYPE_CHANGED_BY_EMAIL  = 'passChangedByEmail';
  const ALERTTYPE_CHANGED_BY_MOBILE = 'passChangedByMobile';
  ...
  public function rules()
  {
    return [
      ...
      ['fprStatus', 'default', 'value' => static::STATUS_NEW],
...
SQLSTR;
    ...
    if (empty($models) == false) {
      ...
      if ($forgotPasswordRequestModel->IsExpired) {
        $forgotPasswordRequestModel->fprStatus = static::STATUS_EXPIRED;
...
SQLSTR;
    ...
    if ($inputType == static::KEYTYPE_EMAIL) {
      $alertModel->alrTypeKey = ForgotPasswordRequestModel::ALERTTYPE_REQUEST_BY_EMAIL;
      $alrInfo['email'] = $normalizedInput;
    } else {
      $alertModel->alrTypeKey = ForgotPasswordRequestModel::ALERTTYPE_REQUEST_BY_MOBILE;
  ...
  static function acceptCode($emailOrMobile, $code, $newPassword)
  {
    ...
    if ($forgotPasswordRequestModel->fprStatus == static::STATUS_APPLIED)
      throw new UnauthorizedHttpException('this code applied before');

    if ($forgotPasswordRequestModel->fprStatus != static::STATUS_SENT)
    ...
    try {
      //1: fpr
      $forgotPasswordRequestModel->fprStatus = static::STATUS_APPLIED;
      ...
      if ($forgotPasswordRequestModel->fprRequestedBy == static::KEYTYPE_EMAIL) {
        $alertModel->alrTarget  = $forgotPasswordRequestModel->user->usrEmail;
        $alertModel->alrTypeKey = ForgotPasswordRequestModel::ALERTTYPE_CHANGED_BY_EMAIL;
        $alrInfo['email'] = $alertModel->alrTarget;
      } else {
        $alertModel->alrTarget  = $forgotPasswordRequestModel->user->usrMobile;
        $alertModel->alrTypeKey = ForgotPasswordRequestModel::ALERTTYPE_CHANGED_BY_MOBILE;
